...
namespace NicoLibrary.nomula
{
    public class Nicorepo : Common.BindableBase
    {
        ...
        public async static Task<ObservableCollection<Nicorepo>> ReadItems()
        {
            ObservableCollection<Nicorepo> nicorepo = null;

            if (App.ViewModel.UserSetting.pcLogined || await App.ViewModel.UserSetting.DeepLogin() == true)//ログイン済み
            {
                try
                {
                    ...
                    if (res != null && res.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        {
                            //パース
                            Nicorepo[] repo = ReadNicorepo(await sr.ReadToEndAsync());
        ...
        private static Nicorepo[] ReadNicorepo(string str)
        {
            Nicorepo[] nicorepo = null;
            ...
            try
            {
                ...
                {
                    nicorepo = new Nicorepo[mc.Count];

                    for (int i = 0; i < mc.Count; i++)
                    {
                        nicorepo[i] = new Nicorepo();
                        nicorepo[i].UserIcon = new Uri(mc[i].Groups["user_icon"].Value, UriKind.Absolute);
                        nicorepo[i].UserName = mc[i].Groups["user_name"].Value;
                        nicorepo[i].VideoTitle = mc[i].Groups["movie_title"].Value;
                        nicorepo[i].VideoThumbnail = new Uri(mc[i].Groups["thumbnail_url"].Value, UriKind.Absolute);
                        nicorepo[i].VideoUrl = new Uri(mc[i].Groups["watch_page"].Value, UriKind.Absolute);
                        nicorepo[i].Date = DateTime.Parse(mc[i].Groups["date"].Value);
                        nicorepo[i].UserID = string.Empty;
                        nicorepo[i].VideoID = string.Empty;
                    }
                }
        ...
        public static Nicorepo[] ReadData()
        {
            Nicorepo[] nicorepo = null;
            ...
            //    if (parsed.Count > 0)
            //    {
            //        nicorepo = new Nicorepo[parsed.Count];
        ...
        public static void AddData(ObservableCollection<Nicorepo> nicorepo)
        {
            ...
            //for (int i = 0, c = nicorepo.Count; i < c; i++)
            //{
